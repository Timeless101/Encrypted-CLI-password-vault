...
def test_validate_password_happy_test(monkeypatch):
    def fake_verify_Password(input_password, database_password, salt):
        return True
    
    monkeypatch.setattr(crypto, "verify_password", fake_verify_Password)

    assert login_logic.validate_password("password", b"password", b"salt") is True
...
def test_validate_password_password_dont_match(monkeypatch):
    ...
    assert login_logic.validate_password("password", b"password", b"salt") is False